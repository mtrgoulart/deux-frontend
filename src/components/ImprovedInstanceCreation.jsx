...
function ImprovedInstanceCreation({ 
  showAddForm, 
  setShowAddForm, 
  onInstanceCreated 
}) {
  ...
  const addApiKeyMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.api_key_id) {
        setFormState(prev => ({ ...prev, api_key: data.api_key_id }));
      }
      setShowAddApiKey(false);
      setNewApiKeyForm({ name: '', api_key: '', secret_key: '', exchange_id: '' });
    }
  });
  ...
  const addStrategyMutation = useMutation({
    mutationFn: async () => {
      const res = await apiFetch('/save_strategy', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(newStrategyForm),
      });
      return res.json();
    },
    onSuccess: (data) => {
      ...
      setNewStrategyForm({ name: '', side: 'buy' });
      setStrategyType('');
    }
  });
          ...
          <div className="space-y-6">
            ...
            <div className="bg-green-500/10 p-4 rounded-lg border border-green-500/30">
              ...
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search for buy strategy..."
                ...
                {strategySuggestionsBuy.length > 0 && (
                  <div className="absolute z-10 w-full mt-1 bg-gray-800 border border-gray-600 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                    {strategySuggestionsBuy.map((strategy) => (
                      <div
                        key={strategy.id}
                        onClick={() => {
                          setFormState(prev => ({ ...prev, strategy_buy: strategy.id }));
                          setStrategySuggestionsBuy([]);
                        }}
                        className="px-4 py-3 hover:bg-green-600/20 cursor-pointer transition-colors"
                      >
                        <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => {
                  setStrategyType('buy');
                  setNewStrategyForm(prev => ({ ...prev, side: 'buy' }));
                  setShowAddStrategy(true);
                }}
                className="mt-2 w-full p-2 border border-dashed border-green-500 rounded text-green-400 hover:bg-green-500/10 transition-all flex items-center justify-center space-x-2"
              >
                <Icons.Add />
            ...
            <div className="bg-red-500/10 p-4 rounded-lg border border-red-500/30">
              ...
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search for sell strategy..."
                ...
                {strategySuggestionsSell.length > 0 && (
                  <div className="absolute z-10 w-full mt-1 bg-gray-800 border border-gray-600 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                    {strategySuggestionsSell.map((strategy) => (
                      <div
                        key={strategy.id}
                        onClick={() => {
                          setFormState(prev => ({ ...prev, strategy_sell: strategy.id }));
                          setStrategySuggestionsSell([]);
                        }}
                        className="px-4 py-3 hover:bg-red-600/20 cursor-pointer transition-colors"
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-medium text-white">{strategy.name}</span>
              ...
              <button
                onClick={() => {
                  ...
                  setNewStrategyForm(prev => ({ ...prev, side: 'sell' }));
                  setShowAddStrategy(true);
                }}
                className="mt-2 w-full p-2 border border-dashed border-red-500 rounded text-red-400 hover:bg-red-500/10 transition-all flex items-center justify-center space-x-2"
              >
                <Icons.Add />
...
export default ImprovedInstanceCreation;
